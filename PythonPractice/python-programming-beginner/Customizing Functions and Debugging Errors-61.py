...
tokenized_vocabulary = tokenize(vocabulary, clean_chars, clean=True)
for x in tokenized_story:
    if x not in tokenized_vocabulary:
        misspelled_words.append(x)
...
def spell_check(vocabulary_file, text_file, special_characters=[",",".","'",";","\n"]):
    misspelled_words = []
    vocab_file = (open(vocabulary_file, "r").read())
    text_file = (open(text_file, "r").read())
    tokenized_vocabulary = tokenize(vocab_file, special_characters)
    tokenized_text = tokenize(text_file, special_characters, True)
    for x in tokenized_text:
        if x not in tokenized_vocabulary and x != '':
            misspelled_words.append(x)
